...
public class AIRecommendationController {
    ...
    private final com.team7.knowledge_gap_platform.service.SkillGapService skillGapService;
    ...
    public AIRecommendationController(
            ...
            com.team7.knowledge_gap_platform.service.SkillGapService skillGapService) {
    ...
    public ResponseEntity<?> generateRecommendations(
        ...
        // Analyze and refresh gaps for the employee directly from PostgreSQL data
        List<SkillGap> gaps;
        try {
        ...
        // Clear existing recommendations for fresh generation
        aiRecommendationService.deleteByEmployeeId(employeeId);
        ...
        for (SkillGap gap : gaps) {
            if (gap.getGapScore() != null && gap.getGapScore() > 0) {
                ...
                try {
                    ...
                    modelSource = "HuggingFace/Llama-3.1-8B-Instruct";
                } catch (Exception e) {
                    recommendationText = String.format(
                        "Based on your gap in %s, we recommend focusing on advanced topics to reach the %s level. " +
                        "1. Review core concepts. 2. Practice through practical projects. 3. Take an advanced training course.",
                        skillName, gap.getRequiredProficiency()
                    );
                    modelSource = "System/Deterministic";
                }

                aiRecommendationService.saveRecommendation(
                    employeeId, gap.getSkillId(), gap.getGapLevel(), recommendationText, modelSource
                );
        ...
        results.sort((r1, r2) -> Integer.compare(r1.getPriority(), r2.getPriority()));
