...
// ============================================================
// DUMMY AUTH (No external context needed)
// ============================================================
const useAuth = () => {
  const logout = () => {
    localStorage.clear();
  ...
  // You can also read user info from localStorage if needed
  const userData = localStorage.getItem('user');
  const user = userData ? JSON.parse(userData) : null;
  const email = user?.email || "admin@okip.com";
  return { email, logout };
};
...
function MiniChart({ path, danger = false }: { path: string; danger?: boolean }) {
  return (
    <svg viewBox="0 0 100 35" className="h-9 w-20" fill="none" preserveAspectRatio="none">
      <path d={path} stroke={danger ? "#ef4444" : "#7c3aed"} strokeWidth="2" strokeLinecap="round" />
    </svg>
  );
}

function KPI({ title, value, change, positive, icon: Icon, iconBg, iconColor, line }: (typeof kpis)[number]) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="mt-4">
        <p className="text-[10px] font-semibold tracking-wide text-slate-500">{title}</p>
  ...
  return (
    <SectionCard title="System Health Overview">
      ...
      <div className="mt-6 flex items-center justify-between border-t border-slate-100 pt-4">
        <span className="text-[10px] text-slate-500">Last updated: 2 mins ago</span>
...
function SystemPerformance() {
  return (
    <SectionCard title="System Performance" subtitle="Resource utilization over the last 6 hours">
    ...
    <SectionCard title="Security Status">
      <div className="space-y-1">
        {items.map(([name, status, Icon]) => {
          ...
          return (
            <div key={name as string} className="flex items-center justify-between border-b border-slate-100 py-3 last:border-0">
              <div className="flex items-center gap-3">
                <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-purple-50">
                  <IconComponent className="h-3.5 w-3.5 text-purple-600" />
                </div>
                <span className="text-xs font-medium text-slate-700">{name as string}</span>
              </div>
              <span className="text-[10px] font-semibold text-emerald-600">{status as string}</span>
            </div>
          );
        })}
      </div>
  ...
  return (
    <SectionCard title="User Management Summary">
      ...
      <div className="mt-5 border-t border-slate-100 pt-4">
        <button className="flex items-center gap-1 text-xs font-semibold text-purple-600">
          Manage Users <ChevronRight className="h-3.5 w-3.5" />
        </button>
      </div>
    </SectionCard>
  );
...
function RoleDistribution() {
  return (
    <SectionCard title="Role Distribution">
      ...
      <div className="mt-5 border-t border-slate-100 pt-4">
        <button className="flex items-center gap-1 text-xs font-semibold text-purple-600">
          Manage Roles <ChevronRight className="h-3.5 w-3.5" />
        </button>
      </div>
    </SectionCard>
  );
...
function RecentSecurityAlerts() {
  return (
    <SectionCard
      ...
      action={<button className="text-[10px] font-semibold text-purple-600">View All</button>}
    >
        ...
        {securityAlerts.map((alert) => {
          ...
          return (
            <div key={alert.title} className="flex gap-3">
              <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${styles[alert.type as keyof typeof styles]}`}>
                <Icon className="h-4 w-4" />
              </div>
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-2">
                  <p className="text-[11px] font-semibold text-slate-800">{alert.title}</p>
                  <span className="shrink-0 text-[9px] text-slate-400">{alert.time}</span>
                </div>
                <p className="mt-1 text-[9px] text-slate-500">{alert.description}</p>
              </div>
            </div>
          );
        })}
        ...
        {stats.map((stat) => {
          ...
          return (
            <div key={stat.label} className="rounded-xl border border-slate-100 p-3">
              ...
              <p className="mt-1 text-[9px] font-semibold text-emerald-600">↑ {stat.change}</p>
            </div>
          );
        })}
      </div>
      <button className="mt-5 flex items-center gap-1 text-xs font-semibold text-purple-600">
        View Training Analytics <ChevronRight className="h-3.5 w-3.5" />
      </button>
    </SectionCard>
  );
}
    ...
    <SectionCard title="System Activity (MTD)">
      <div className="space-y-0">
        {[
          ...
          <div key={activity} className="grid grid-cols-[1fr_80px_65px] items-center border-b border-slate-100 py-3 last:border-0">
            <span className="text-[10px] text-slate-600">{activity}</span>
            <span className="text-right text-[11px] font-bold text-slate-800">{count}</span>
            <span className="text-right text-[9px] font-semibold text-emerald-600">↑ {change.replace("+", "")}</span>
          </div>
        ))}
      </div>
      <button className="mt-4 flex items-center gap-1 text-xs font-semibold text-purple-600">
        View Activity Logs <ChevronRight className="h-3.5 w-3.5" />
      </button>
    </SectionCard>
  );
}
    ...
    <SectionCard title="Database & Backup Status">
      <div className="space-y-2">
        {items.map(([name, value, color, Icon]) => {
          const IconComponent = Icon as React.ElementType;
          return (
            <div key={name as string} className="flex items-center justify-between rounded-xl border border-slate-100 px-3 py-3">
              <div className="flex items-center gap-2">
                <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-purple-50">
                  <IconComponent className="h-3.5 w-3.5 text-purple-600" />
                </div>
                <span className="text-[10px] text-slate-600">{name as string}</span>
              </div>
              <span className={`text-[9px] font-semibold ${color as string}`}>{value as string}</span>
            </div>
          );
        })}
      </div>
      <button className="mt-4 flex items-center gap-1 text-xs font-semibold text-purple-600">
        Backup Management <ChevronRight className="h-3.5 w-3.5" />
      </button>
    </SectionCard>
  );
}
        ...
        <table className="w-full min-w-[650px]">
          <thead>
            <tr className="border-b border-slate-100 text-left">
              <th className="pb-3 text-[9px] font-semibold text-slate-400">TIME</th>
              <th className="pb-3 text-[9px] font-semibold text-slate-400">USER</th>
              <th className="pb-3 text-[9px] font-semibold text-slate-400">ACTION</th>
              <th className="pb-3 text-[9px] font-semibold text-slate-400">MODULE</th>
              <th className="pb-3 text-right text-[9px] font-semibold text-slate-400">STATUS</th>
            </tr>
          </thead>
          <tbody>
            {activities.map((activity) => (
              <tr key={`${activity.time}-${activity.user}`} className="border-b border-slate-50 last:border-0">
                <td className="py-3 text-[9px] text-slate-500">
                  <div>{activity.time}</div>
                  <div className="text-[8px] text-slate-400">{activity.date}</div>
                </td>
                <td className="py-3 text-[10px] font-semibold text-slate-700">{activity.user}</td>
                <td className="py-3 text-[10px] text-slate-600">{activity.action}</td>
                <td className="py-3 text-[9px] font-medium text-slate-500">{activity.module}</td>
                <td className="py-3 text-right">
                  <span className="rounded-full bg-emerald-50 px-2 py-1 text-[8px] font-semibold text-emerald-600">Success</span>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        {actions.map(([label, Icon]) => {
          ...
          return (
            ...
            >
              ...
              <span className="mt-3 text-center text-[10px] font-semibold text-slate-700">{label as string}</span>
            </button>
          );
        })}
      </div>
...
function Sidebar({ mobileOpen, setMobileOpen }: { mobileOpen: boolean; setMobileOpen: React.Dispatch<React.SetStateAction<boolean>> }) {
  const menu = [
      ...
      >
        <div className="flex h-[68px] items-center border-b border-slate-100 px-7">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xl font-bold tracking-tight text-slate-900">OKIP</span>
          </div>
          <button onClick={() => setMobileOpen(false)} className="ml-auto rounded-lg p-1 text-slate-400 lg:hidden">
            <X className="h-5 w-5" />
          </button>
        </div>
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center gap-3 rounded-xl px-4 py-2.5 text-left text-[12px] font-medium transition ${
                    index === 0 ? "bg-purple-50 text-purple-600" : "text-slate-600 hover:bg-slate-50 hover:text-purple-600"
                  }`}
                >
        ...
        <div className="border-t border-slate-100 p-4">
          ...
          <button className="mt-4 flex w-full items-center gap-2 px-4 text-[10px] text-slate-400">
            <ChevronRight className="h-3.5 w-3.5 rotate-180" /> Collapse Sidebar
          </button>
        </div>
      </aside>
    </>
...
function Header({ setMobileOpen }: { setMobileOpen: React.Dispatch<React.SetStateAction<boolean>> }) {
  // Using the dummy auth defined at the top
  ...
  return (
    <header className="flex h-[68px] items-center justify-between border-b border-slate-200 bg-white px-5 lg:px-7">
      <div className="flex items-center gap-3">
        <button onClick={() => setMobileOpen(true)} className="rounded-lg p-2 text-slate-500 lg:hidden">
          <Menu className="h-5 w-5" />
        </button>
        <div className="hidden items-center gap-2 text-xs text-slate-500 sm:flex">
          <span>Dashboard</span> <span>/</span> <span className="font-semibold text-slate-900">Overview</span>
        </div>
      </div>
      <div className="flex items-center gap-3">
        <div className="hidden h-10 w-[220px] items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 md:flex">
          <Search className="h-4 w-4 text-slate-400" />
          <input className="w-full bg-transparent text-xs outline-none placeholder:text-slate-400" placeholder="Search users, skills, reports..." />
        </div>
        <button className="relative rounded-xl p-2 text-slate-500 hover:bg-slate-50">
          <Bell className="h-5 w-5" />
          <span className="absolute right-1 top-1 h-2 w-2 rounded-full bg-red-500" />
        </button>
        <button onClick={() => setDarkMode(!darkMode)} className="rounded-xl p-2 text-slate-500 hover:bg-slate-50">
        ...
        <div className="flex items-center gap-2 border-l border-slate-200 pl-3">
          ...
          <div className="hidden sm:block">
            <p className="text-[11px] font-bold text-slate-800">Daniel Morgan</p>
            <p className="text-[9px] text-slate-500">System Administrator</p>
          </div>
          <button onClick={logout} title={`Logout ${email ?? ""}`} className="ml-1 rounded-lg p-1 text-slate-400 hover:bg-slate-50">
        ...
        <div className="min-w-0 flex-1">
          ...
          <main className="mx-auto max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8">
            ...
            <div className="mb-6 flex flex-col justify-between gap-4 xl:flex-row xl:items-start">
              <div>
                ...
                <p className="mt-1 max-w-2xl text-xs leading-5 text-slate-500">
                  Monitor system health, manage users, roles, and security while ensuring optimal platform performance.
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                ...
                <button className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-[11px] font-semibold text-slate-700 shadow-sm hover:border-purple-200 hover:text-purple-600">
                  <UserRoundCog className="h-4 w-4" /> Role Management
                </button>
                <button className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-[11px] font-semibold text-slate-700 shadow-sm hover:border-purple-200 hover:text-purple-600">
                ...
                <button className="flex items-center gap-2 rounded-xl bg-purple-600 px-4 py-2.5 text-[11px] font-semibold text-white shadow-lg shadow-purple-200 hover:bg-purple-700">
                  <FileBarChart className="h-4 w-4" /> Generate System Report
                </button>
              </div>
            </div>
            ...
            <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="mb-5 flex items-start justify-between">
                <div>
                  <h2 className="text-base font-bold text-slate-900">🔥 Department Competency Heatmap</h2>
                ...
                <table className="w-full min-w-[600px] border-collapse text-sm">
                  <thead>
                    <tr className="border-b border-slate-200">
                      <th className="py-2 text-left font-semibold text-slate-600">Department</th>
                      <th className="py-2 text-center font-semibold text-slate-600">Cloud</th>
                      <th className="py-2 text-center font-semibold text-slate-600">AI/ML</th>
                      <th className="py-2 text-center font-semibold text-slate-600">Security</th>
                      <th className="py-2 text-center font-semibold text-slate-600">Data</th>
                      <th className="py-2 text-center font-semibold text-slate-600">Leadership</th>
                    </tr>
                  </thead>
                  <tbody>
                    {[
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                <span className="inline-flex items-center gap-1.5 rounded-full bg-red-100 px-3 py-1 text-xs font-medium text-red-700">
                  <span className="h-2.5 w-2.5 rounded-full bg-red-500"></span> High Gap (&lt;60%)
                </span>
                <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-100 px-3 py-1 text-xs font-medium text-amber-700">
                  <span className="h-2.5 w-2.5 rounded-full bg-amber-500"></span> Medium Gap (60-80%)
                </span>
                <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-100 px-3 py-1 text-xs font-medium text-emerald-700">
                  <span className="h-2.5 w-2.5 rounded-full bg-emerald-500"></span> Low Gap (&gt;80%)
                </span>
              </div>
              <div className="mt-4 border-t border-slate-100 pt-4 text-[10px] text-slate-400">
                Data reflects current skill gaps across departments. Based on the latest assessments.
              </div>
            </div>

            {/* FOOTER */}
            <div className="py-6 text-center">
              <p className="text-[10px] text-slate-400">OKIP • Organizational Knowledge Gap Intelligence Platform</p>
            </div>
          </main>
        </div>
      </div>
