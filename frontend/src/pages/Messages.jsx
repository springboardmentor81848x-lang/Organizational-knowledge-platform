...
export default function Messages() {
  ...
  const requestedMentorshipId = searchParams.get('mentorshipId');
  ...
  // 1. Fetch active chats for the current employee
  const fetchActiveChats = async (selectMentorshipId = null) => {
    if (!employeeId) {
    ...
    try {
      const response = await axios.get(
        `${API_BASE_URL}/messages/active-chats/${employeeId}`,
        getHeaders()
      );
      ...
      const targetId = selectMentorshipId || requestedMentorshipId;
      if (targetId && chats.length > 0) {
        const found = chats.find((c) => String(c.mentorshipId) === String(targetId));
        if (found) {
  ...
  const fetchMessages = async (mentorshipId, isBackground = false) => {
    if (!mentorshipId) return;

    try {
      if (!isBackground) setLoadingMessages(true);
      const response = await axios.get(
        `${API_BASE_URL}/messages/mentorship/${mentorshipId}?employeeId=${employeeId}`,
        getHeaders()
      );
      ...
      if (!isBackground) {
        setError(err.response?.data?.message || err.response?.data || 'Failed to load messages.');
      }
    } finally {
  ...
  // Initial load
  useEffect(() => {
    fetchActiveChats(requestedMentorshipId);
  }, [employeeId, requestedMentorshipId]);

  // When selected chat changes, load its messages
  useEffect(() => {
    if (selectedChat?.mentorshipId) {
      setError('');
      fetchMessages(selectedChat.mentorshipId);
      // Sync URL query parameter
      setSearchParams({ mentorshipId: String(selectedChat.mentorshipId) }, { replace: true });
    } else {
      setMessages([]);
    }
  }, [selectedChat?.mentorshipId]);

  // Polling for real-time messaging updates (every 3.5 seconds)
  useEffect(() => {
    if (!selectedChat?.mentorshipId) return;

    const interval = setInterval(() => {
      fetchMessages(selectedChat.mentorshipId, true);
    }, 3500);
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/messages`, payload, getHeaders());

      if (response.data) {
        setMessages((prev) => [...prev, response.data]);
        setTimeout(scrollToBottom, 50);
        // Refresh chat list snippet
        fetchActiveChats(selectedChat.mentorshipId);
      }
    } catch (err) {
  ...
  // Key handler for Enter key (Shift+Enter allows newline)
  ...
  const filteredChats = useMemo(() => {
    ...
    return activeChats.filter(
      (c) =>
        (c.peerName && c.peerName.toLowerCase().includes(q)) ||
        (c.skillName && c.skillName.toLowerCase().includes(q)) ||
        (c.peerEmployeeId && c.peerEmployeeId.toLowerCase().includes(q))
    );
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen">
        ...
        <div className="flex-1 flex overflow-hidden p-4 md:p-6">
          <div className="flex-1 flex bg-white rounded-2xl border border-slate-200/90 shadow-lg overflow-hidden">
            {/* ========================================================
                LEFT PANEL: ACTIVE PEER CHATS LIST
            ======================================================== */}
              ...
              <div className="p-4 border-b border-slate-200 bg-white">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h2 className="text-base font-bold text-slate-900 leading-tight">Peer Messages</h2>
                      <p className="text-[11px] text-slate-500">Accepted Mentorship Chats</p>
                    </div>
                  </div>
                  <button
                    onClick={() => fetchActiveChats(selectedChat?.mentorshipId)}
                    title="Refresh Chats"
              ...
              <div className="flex-1 overflow-y-auto divide-y divide-slate-100">
                {loadingChats ? (
                  <div className="p-8 text-center text-slate-400 text-xs flex flex-col items-center gap-2">
                    ...
                    <span>Loading active peer chats...</span>
                  </div>
                ) : filteredChats.length === 0 ? (
                  <div className="p-8 text-center text-slate-500">
                    <UserCheck size={36} className="mx-auto text-slate-300 mb-2" />
                    <p className="text-xs font-semibold text-slate-700">No active chats found</p>
                  ...
                  filteredChats.map((chat) => {
                    const isSelected = selectedChat?.mentorshipId === chat.mentorshipId;
                    const peerInitials = chat.peerName
                      ? chat.peerName
                          .split(' ')
                          .map((n) => n[0])
                          .slice(0, 2)
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-1.5 mt-0.5">
                            ...
                            <span className="text-[10px] font-medium text-slate-400">
                              {chat.isCurrentUserMentor ? 'Mentee' : 'Mentor'}
                            </span>
                          </div>
            ...
            {/* ========================================================
                RIGHT PANEL: CHAT WINDOW (WhatsApp style)
            ======================================================== */}
            <div className="flex-1 flex flex-col bg-[#efeae2]/30 min-w-0 relative">
              {selectedChat ? (
                <>
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          <h3 className="font-bold text-slate-900 text-sm truncate">
                            Chat with {selectedChat.peerName}
                          </h3>
                          <span className="hidden sm:inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 text-[10px] font-semibold">
                            <ShieldCheck size={11} /> Accepted
                          </span>
                        </div>
                        <p className="text-xs text-slate-500 truncate flex items-center gap-1.5 mt-0.5">
                          <span className="font-medium text-blue-700">{selectedChat.skillName} Mentoring</span>
                          <span>•</span>
                          <span>{selectedChat.peerDesignation || 'Peer Expert'}</span>
                          {selectedChat.peerDepartment && (
                            <>
                              <span>•</span>
                              <span>{selectedChat.peerDepartment}</span>
                            </>
                          )}
                        </p>
                      </div>
                  ...
                  <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-3">
                    ...
                    <div className="flex justify-center my-2">
                      <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-2 text-center text-[11px] text-amber-800 shadow-sm max-w-md">
                        🔒 Peer mentoring chat for <strong>{selectedChat.skillName}</strong> is active and secured. You and{' '}
                        <strong>{selectedChat.peerName}</strong> can now exchange mentoring advice and questions.
                      </div>
                    </div>
                    ...
                    {loadingMessages ? (
                      ...
                      <div className="text-center py-12">
                        ...
                        <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
                          Start the conversation with {selectedChat.peerName} by asking a question or introducing your learning goals!
                        </p>
                      </div>
                    ) : (
                      messages.map((msg, index) => {
                        const isMe =
                          String(msg.senderEmployeeId) === String(employeeId) ||
                            ...
                            >
                              ...
                              {!isMe && (
                                <p className="text-[11px] font-bold text-blue-600 mb-0.5">
                                  {msg.senderName || selectedChat.peerName}
                                </p>
                              )}
                              ...
                              >
                                ...
                                {isMe && (
                                  <CheckCheck
                                    size={13}
                                    className={msg.readStatus ? 'text-emerald-300' : 'text-blue-200'}
                  ...
                  {/* Input Bar (WhatsApp-like bottom toolbar) */}
                  <div className="p-3 md:p-4 bg-white border-t border-slate-200">
                    ...
                    <p className="text-[10px] text-slate-400 mt-1 pl-2">
                      Tip: Press <kbd className="bg-slate-100 px-1 rounded border">Enter</kbd> to send, or <kbd className="bg-slate-100 px-1 rounded border">Shift + Enter</kbd> for a new line.
                    </p>
                  </div>
                </>
              ) : (
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                  ...
                  <h3 className="text-lg font-bold text-slate-900">Peer Mentorship Messaging</h3>
